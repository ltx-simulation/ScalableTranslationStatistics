...
model TwoDimNLSpring
  ...
  parameter  Modelica.Units.SI.Position s0=1;
...
equation
  ...
  //f_res = [c,0.1*c;0.2*c,c]*(s_rel.*s_rel.*s_rel/s0^2);
 if insert_Jacobian then
    limit(f,1e20)= [c,0.1*c;0.2*c,c]*(s_rel.*s_rel.*s_rel/s0^2);

    //f[1] = limit(f_res[1],1e20);
    //f[2] = f_res[2];
 else
    //f=f_res;
    f= [c,0.1*c;0.2*c,c]*(s_rel.*s_rel.*s_rel/s0^2);
  end if;
