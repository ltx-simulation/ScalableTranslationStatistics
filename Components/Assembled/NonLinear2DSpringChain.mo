...
model NonLinear2DSpringChain

  extends Partial.Partial_SpringChain( N= integer(ceil((num_springs+1)/2)),
  redeclare Subcomponents.TwoDimNLSpring spring[N](c=c,insert_Jacobian=Jacs));
  ...
  //parameter Integer num_springs=4 "number of springs";
  // Subcomponents.TwoDimNLSpring spring[N](c=c,insert_Jacobian=Jacs);
...
protected
  ...
  //parameter Integer N=if insert_Jacobian==true then integer(ceil((num_springs+1)/2)) else integer(ceil(num_springs/2));
  //parameter Integer N= integer(ceil((num_springs+1)/2));
