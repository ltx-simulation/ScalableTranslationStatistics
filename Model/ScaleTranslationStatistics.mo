...
model ScaleTranslationStatistics
  ...
  replaceable Components.Subcomponents.LinearSleepySpring
    stiffSleepySpring(
    c=c_stiff,
    waiting_time=sleeping_time,
    compiler_type=compilerType) constrainedby
    Components.Partial.PartialSleepySpring                                                     "linear spring connecting a single mass"
  ...
  replaceable Components.Assembled.NonLinearSpringChain springChain[num_NL](
    each c_1=c,
    each c_2=c,
    each s_start=s_initial,
    num_springs=NL_equations,
    insert_Jacobian=include_Jacobi[:]) constrainedby
    Components.Partial.Partial_SpringChain "nonlinear spring chains"
    annotation (Placement(transformation(extent={{-46,46},{-6,86}})));

  //   linear spring chains
  Components.Assembled.LinearSpringChain linearSpringChain[num_Lin](
    each c_1=c,
    each c_2=c,
    each s_start=s_initial,
